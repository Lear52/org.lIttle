...
import org.little.web.filter.config.Constants;
import org.little.web.filter.config.SpnegoFilterConfig;
import org.little.web.filter.impl.SpnegoAuthScheme;
import org.little.web.filter.impl.SpnegoHttpServletResponse;
import org.little.web.filter.impl.SpnegoPrincipal;
import org.little.web.filter.impl.SpnegoProvider;
import org.little.web.filter.impl.util.Base64;
 ...
 * not occur but instead simply returns the <code>System.getProperty("util.name")</code>
...
public class SpnegoAuthenticator {
       ...
       private static final    Lock          LOCK    = new ReentrantLock();      /** GSSContext is not thread-safe. */
       private static final    GSSManager    MANAGER = GSSManager.getInstance(); /** Default GSSManager. */
       private final transient boolean       allowBasic;                         /** Flag to indicate if BASIC Auth is allowed. */
       private final transient boolean       allowDelegation;                    /** Flag to indicate if credential delegation is allowed. */
       private final transient boolean       allowLocalhost;                     /** Flag to skip auth if localhost. */
       private final transient boolean       allowUnsecure;                      /** Flag to indicate if non-SSL BASIC Auth allowed. */
       private final transient boolean       promptIfNtlm;                       /** Flag to indicate if NTLM is accepted. */
       private final transient String        clientModuleName;                   /** Login Context module name for client auth. */
       private final transient LoginContext  loginContext;                       /** Login Context server uses for pre-authentication. */
       private final transient GSSCredential serverCredentials;                  /** Credentials server uses for authenticating requests. */
       private final transient KerberosPrincipal serverPrincipal;                /** Server Principal used for pre-authentication. */ 
       /**
       ...
       public SpnegoAuthenticator(final String loginModuleName, final SpnegoFilterConfig config) throws LoginException, GSSException, PrivilegedActionException {
              ...
              logger.trace("loginModuleName=" + loginModuleName);
              ...
              if(hasUsername        ) this.loginContext = new LoginContext(loginModuleName, SpnegoProvider.getUsernamePasswordHandler(username, config.getPreauthPassword()));
              else 
              if (config.useKeyTab()) this.loginContext = new LoginContext(loginModuleName);
              else                    throw new IllegalArgumentException("Must provide a username/password or specify a keytab file");
       ...
       /**
        * Returns the KerberosPrincipal of the util/client making the HTTP request.
        * 
        * <p>
       ...
       public SpnegoPrincipal authenticate(final HttpServletRequest req, final SpnegoHttpServletResponse resp) throws GSSException, IOException {
             ...
             if (scheme.isBasicScheme()) {
                 // check if we allow Basic Auth AND if can be un-secure
                 if (basicSupported) {
                     principal = doBasicAuth(scheme, resp);
                 } else {
                     logger.error("allowBasic=" + this.allowBasic + "; allowUnsecure=" + this.allowUnsecure + "; req.isSecure()=" + req.isSecure());
                     throw new UnsupportedOperationException("Basic Auth not allowed" + " or SSL required.");
                 }
            
             // Unsupported scheme
             } 
             else throw new UnsupportedOperationException("scheme=" + scheme);
       ...
       public void dispose() {
              ...
              if (null != this.loginContext) {
                  try {
                      ...
                      logger.error("Logout failed."+lex);
                  }
              }
       }     
       ...
       private SpnegoPrincipal doBasicAuth(final SpnegoAuthScheme scheme, final SpnegoHttpServletResponse resp) throws IOException {
               ...
               if (0 == data.length) {
                   logger.error("Basic Auth data was NULL.");
                   return null;
               }
               ...
               try {
                   ...
                   logger.info(lex.getMessage() + ": Login failed. username=" + username);
       ...
       private SpnegoPrincipal doLocalhost() {
           final String username = System.getProperty("util.name");
       ...
       private SpnegoPrincipal doSpnegoAuth(
               ...
               if (0 == gss.length) {
                   logger.error("GSS data was NULL.");
                   return null;
               }
               ...
               try {
                   ...
                   if (!context.isEstablished()) {
                       logger.trace("context not established");
                       resp.setStatus(HttpServletResponse.SC_UNAUTHORIZED, true);
                       return null;
                   }
