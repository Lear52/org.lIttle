 ...
 * util information or from where the util information is stored. An
 ...
 * roles, groups, attributes, resources, util information etc. is
 ...
 * based on attributes of the util. Usage semantics will be in the
 ...
 * class connects to an LDAP server for the purpose of obtaining util attribute/
 ...
 * for a more expressive policy statement. e.g. 1) A util has access if they are
 * in <i>this</i> AD Group and belong in one of <i>these</i> departments, or 
 * 2) a util has access if they are in <i>this</i> email distribution list
 * and in one of <i>these</i> AD Groups or is in one <i>these</i> departments, 
 * or 3) a util can see <i>the edit button</i> if they are in <i>this</i>
 * AD Group and in one of <i>these</i> other AD Groups.
 * </p>
 * 
 * <p>
 * <b>Example Usage 1:</b><br />
 * A new ticketing application where every util of the application has the same
 * access rights and the only requirement is that they have logged-in to their 
 * workstation/computer. Under this scenario, every util must first authenticate (authN)
 * but since every authN util has the same access rights to the ticketing application,
 ...
 * </pre>
 * 
 * In the above example, if the util dfelix has at least one of those attributes (in this
 ...
 * identify a util or a set of users. For example, a job title named "Team Lead", a
 ...
 * lists, active directory groups, util location, etc. are attribute sets. An attribute set is
 * commonly and sometimes indirectly specified in a util store (database/ldap/active directory/etc.)
 * as util information properties, system roles/groups, company distribution lists/groups, etc.
 * </p>
 * 
 * <p>
 ...
 * (Example Usage 3), edit/admin buttons will be enabled only if the util dfelix has the
 ...
 * if some policy statement D, specifies that a util MUST be in the IT Group AND that the
 * util must be in the Performance Team email distribution list; however the attribute
 ...
 * util-defined resource label named admin-buttons. Concretely, the resource label
 * admin-buttons could have been assigned the attributes IT Group, Biz. Analyst, and Developer.
 * </p>
 * 
 * <p>
 * To see how a web application/service can leverage util access controls, as well as see
