 ...
 * 
 * <p>
 * In simple terms, roles are attributes that belong to a util and that an attribute
 ...
 * to a role_group.  In addition, a role_group can be concretely assigned to a util,
 * whereas an attribute set can NOT be concretely assigned to a util.
 * </p>
 * 
 * <p>
 ...
 * This example will assume Active Directory (AD) as the data store for util information.
 ...
 * that the department attribute in AD's util profile is populated
 * for each util. Example values in the department attribute set might be
 ...
 * util but you can't assign an attribute set (e.g. department to a util). For example,
 ...
 * if the util is in the department named HR or the AD Group named "File share Access"
 ...
 * </pre>
 * 
 * In the above example, if the util has the attribute File Share Access
 ...
 * is to use a util-defined resource label. Resource labels are optional and hence must
 ...
 * util-defined resource label named finance-links. Concretely, given the previous example,
 ...
 * (authN) util, the {@link javax.servlet.http.HttpServletRequest} interface also defines
 * an <code>isUserInRole</code> method that  
 * "<i>returns a boolean indicating whether the authenticated util is included in the
