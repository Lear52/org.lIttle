...
public class SpnegoSOAPConnection extends SOAPConnection {
       ...
       private static final Logger logger =LoggerFactory.getLogger(SpnegoSOAPConnection.class);
        ...
        * @param integrity
        */
       public SpnegoSOAPConnection(final GSSCredential creds, final boolean dispose, final boolean confidential, final boolean integrity) {
           super();
           this.conn = new SpnegoHttpURLConnection(creds, dispose);
        ...
        * @throws LoginException 
        */
       public SpnegoSOAPConnection(final String loginModuleName,final String username, final String password) throws LoginException {
       ...
       public final SOAPMessage call(final SOAPMessage request, final Object endpoint) throws SOAPException {
           ...
           try {
               ...
               if (null == contentType) {
                   ...
                   // util specified as a MIME header so add it as an HTTP header parameter
       ...
       private Document parse(final InputStream stream) throws SAXException, IOException, ParserConfigurationException {
           
           this.documentFactory.setNamespaceAware(true);
           
           final Document document = this.documentFactory.newDocumentBuilder().parse(stream);
           
           return document;
       }
       
       private SOAPMessage transform(final Node soapBody) throws SOAPException, IOException, TransformerException, SAXException, ParserConfigurationException {
     
           final SOAPMessage message = messageFactory.createMessage();
           final Transformer transformer = TransformerFactory.newInstance().newTransformer();
           final NodeList    children = soapBody.getChildNodes();
           
           logger.trace("number of children=" + children.getLength());
     
           for (int i = 0; i < children.getLength(); i++) {
               
               logger.trace("child[" + i + "]=" + children.item(i).getLocalName());
