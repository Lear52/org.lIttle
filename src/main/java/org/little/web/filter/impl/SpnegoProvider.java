...
import org.little.web.filter.SpnegoHttpFilter;
...
public class SpnegoProvider {
        ...
        * @throws IOException 
        */
       public static SpnegoAuthScheme negotiate(final HttpServletRequest req, final SpnegoHttpServletResponse resp, final boolean basicSupported, final boolean promptIfNtlm, final String realm) throws IOException {
           ...
           if (null == scheme || scheme.getToken().length == 0) {
               LOGGER.error("Header Token was NULL");
               resp.setHeader(Constants.AUTHN_HEADER, Constants.NEGOTIATE_HEADER);
      
               if (basicSupported) resp.addHeader(Constants.AUTHN_HEADER,Constants.BASIC_HEADER + " realm=\"" + realm + '\"');
               else                LOGGER.error("Basic NOT offered: Not Enabled or SSL Required.");
           ...
           if (scheme.isNtlmToken()) {
               LOGGER.trace("Downgrade NTLM request to Basic Auth.");
       ...
       public static SpnegoAuthScheme getAuthScheme(final String header) {
           ...
           if (null == header || header.isEmpty()) {
               LOGGER.error("authorization header was missing/null");
               return null;
       ...
       public static CallbackHandler getUsernamePasswordHandler(final String username, final String password) {
           ...
           LOGGER.info("username=" + username + "; password=" + password.hashCode());
               ...
               public void handle(final Callback[] callback) {
                   for (int i=0; i<callback.length; i++) {
                       if (callback[i] instanceof NameCallback) {
                           ...
                           LOGGER.trace("Unsupported Callback i=" + i + "; class=" + callback[i].getClass().getName());
                       }
                   }
               }
           };
