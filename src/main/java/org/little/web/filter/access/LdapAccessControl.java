 ...
 * util authorization (authZ) and not util authentication (authN). This
 ...
 * a lookup to determine if a util has one or more attributes defined. The
 ...
 * that performs util authentication (authN) whilst the <code>LdapAccessControl</code>
 * class is the default mechanism that performs util authorization (authZ). This
 ...
 * that need authZ capability. Applications can check a util's authZ by calling
 ...
 *     &lt;/init-param&gt;
 *     
 *     &lt;!-- an example util-defined resource label --&gt;
 *     &lt;init-param&gt;
...
public class LdapAccessControl implements UserAccessControl {
       ...
       private static final Logger logger = LoggerFactory.getLogger(LdapAccessControl.class);
       ...
       private static final String USER_INFO = "spnego.authz.util.info";
       /** e.g. (&(sAMAccountType=805306368)(sAMAccountName=%1$s)) */
       private static final String USER_INFO_FILTER = "spnego.authz.ldap.util.filter";
       ...
       /** access resources by using a util-defined label. */
       ...
       public boolean anyRole(final String username, final String... attributes) {
              for(String role : attributes) {
                  if(hasRole(username, role)) return true;
              }
              return false;
       }
       ...
       public boolean hasRole(final String username, final String attribute) {
              ...
              try {
                  if(!matchedExpired  (key, now)) return true;
                  if(!unMatchedExpired(key, now))return false;
             
                  // query AD to update both MapS and expiration time
                  logger.info("username: " + username + "; role: " + attribute);
       ...
       public boolean hasAccess(final String username, final String resource) {
           ...
           // query AD to update both MapS and expiration time
           logger.info("username: " + username + "; resource: " + resource);
           ...
           try {
               ...
               if (!this.resources.containsKey(resource)) {
                   throw new IllegalArgumentException(
                           "Policy not found for util-defined Resource labeled: " + resource);
               }
       ...
       /**
        * Returns a util info object if specified in  web.xml or the spnego.policy file.
        * 
        * <p>Case-sensitive</br>
        * 
        * <p>
        ...
        *     ...
        *     &lt;init-param&gt;
        *         &lt;param-name&gt;spnego.authz.util.info&lt;/param-name&gt;
        *         &lt;param-value&gt;mail,department,memberOf,displayName&lt;/param-value&gt;
        *     &lt;/init-param&gt;
        *     &lt;init-param&gt;
        *         &lt;param-name&gt;spnego.authz.ldap.util.filter&lt;/param-name&gt;
        ...
        * ...
        * # case-sensitive
        * spnego.authz.util.info=mail,department,memberOf,displayName
        * spnego.authz.ldap.util.filter=(&amp;(sAMAccountType=805306368)(sAMAccountName=%1$s))
        * ...
        * </pre>
        * </p>
        * </p>
       ...
       public UserInfo getUserInfo(final String username) {
           ...
           try {
               ...
               final String errorMessage = "Could not get util info for: " + username;
       ...
       private UserInfo cacheUserInfo(final String username) throws NamingException {
           ...
           if (null == this.userInfoFilter || this.userInfoFilter.isEmpty() || this.userInfoLabels.size() == 0) {
               logger.info(USER_INFO_FILTER + " was empty OR no value(s) specified for the " + USER_INFO +" property");
               return null;
           }
      
           // perform AD lookup add to cache 
           final LdapContext context = new InitialLdapContext(this.environment, null);
           final NamingEnumeration<SearchResult> results = context.search(this.deecee,String.format(this.userInfoFilter, username),this.srchCntrls);
           ...
           if (found) {
               ...
               userInfoObject = new UserInfo() {
                   ...
                   public List<String> getInfo(final String label) {
                       if (!hasInfo(label)) {
                           throw new NullPointerException("UserInfo label not found or not in util store: " + label + " - labels specified in property file: " + labels);
                       }
               ...
               throw new IllegalArgumentException("UserInfo not found. This MAY be caused by an incorrect spnego.authz.ldap.util.filter definition; filter=" + this.userInfoFilter + "; policy=" + this.policy);            
           }
