...
import org.little.smtpcln.handler.SmtpCommandHandler;
import org.little.smtpcln.handler.SmtpInitializer;
...
import org.little.smtpcln.rr.SmtpRequests;
...
public class SmtpClient{
        ...
        private SmtpInitializer start_handler;
        ...
        public int start() {
               start_handler=new SmtpInitializer();
               SmtpCommandHandler command=start_handler.getCommandHandler();
               logger.trace("begin queue");

               SmtpElement request;
               request=SmtpRequests.ehlo("D-SHADRINAV");             command.addQueue(request);
               request=SmtpRequests.help(null);                      command.addQueue(request);
               request=SmtpRequests.auth("LOGIN");                   command.addQueue(request);
               String username=_Base64.StrToBase64("av@vip.cbr.ru");
               String password=_Base64.StrToBase64("123");
               request=SmtpRequests.empty(username);                 command.addQueue(request);
               request=SmtpRequests.empty(password);                 command.addQueue(request);
               request=SmtpRequests.vrfy("av@vip.cbr.ru");           command.addQueue(request);
               request=SmtpRequests.vrfy("iap@vip.cbr.ru");          command.addQueue(request);
              
               request=SmtpRequests.noop();                          command.addQueue(request);
              
               //MAIL FROM:<av@vip.cbr.ru>                           
               request=SmtpRequests.mail("av@vip.cbr.ru");           command.addQueue(request);
               //RCPT TO:<av@vip.cbr.ru>
               request=SmtpRequests.rcpt("av@vip.cbr.ru");           command.addQueue(request);
               request=SmtpRequests.rcpt("iap@vip.cbr.ru");          command.addQueue(request);
               request=SmtpRequests.data();                          command.addQueue(request);
               
               byte [] _body=body.getBytes();                        ByteBuf data=Unpooled.wrappedBuffer(_body);
               SmtpContent _request=new LastSmtpContent(data);       command.addQueue(_request);_request.set(_body);
               
               request=SmtpRequests.quit();                          command.addQueue(request);
        ...
        public void run() {
                ...
                try {
                     ...
                     ChannelFuture f = boot_strap.connect(new InetSocketAddress(remote_host, remote_port)).sync();
