...
public class serverAPI{
    ...
    public serverAPI(String broker, String service, boolean verbose){
        assert (broker != null);
        assert (service != null);
        this.broker = broker;
     ...
     * @param msg
     */
    void sendToBroker(CMD command, String option, ZMsg msg){
         msg = msg != null ? msg.duplicate() : new ZMsg();
        
         // Stack protocol envelope to start of message
         if (option != null) msg.addFirst(new ZFrame(option));
        
         msg.addFirst(command.newFrame());
         msg.addFirst(CMD.W_WORKER.newFrame());
    ...
    void reconnectToBroker(){
        ...
        sendToBroker(CMD.W_READY, service, null);
    ...
    public ZMsg receive(ZMsg reply){
        ...
        if (reply != null) {
            assert (replyTo != null);
            reply.wrap(replyTo);
            sendToBroker(CMD.W_REPLY, null, reply);
            reply.destroy();
        }
        expectReply = true;
        ...
        while (!Thread.currentThread().isInterrupted()) {
            ...
            if (items.poll(timeout) == -1)break; // Interrupted
            ...
            if (items.pollin(0)) {
                ...
                assert (CMD.W_WORKER.frameEquals(header));
                header.destroy();

                ZFrame command = msg.pop();
                if (CMD.W_REQUEST.frameEquals(command)) {
                ...
                else if (CMD.W_HEARTBEAT.frameEquals(command)) {
                    // Do nothing for heartbeats
                }
                else if (CMD.W_DISCONNECT.frameEquals(command)) {
                    reconnectToBroker();
                }
                else {
            ...
            if (System.currentTimeMillis() > heartbeatAt) {
                sendToBroker(CMD.W_HEARTBEAT, null, null);
                heartbeatAt = System.currentTimeMillis() + heartbeat;
            }
            items.close();
        }

        if(Thread.currentThread().isInterrupted())log.format("W: interrupt received, killing worker\n");
    ...
    public void setReconnect(int reconnect){this.reconnect = reconnect;}
