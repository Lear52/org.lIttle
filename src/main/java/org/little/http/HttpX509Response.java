...
import org.little.store.fsstore.lFSFolder;
import org.little.store.fsstore.lFSMessage;
...
public class HttpX509Response {
       ...
       public String _getError(String err) {
       ...
       //       return sfd.format(d);
       //}
       public void sendTxt(ChannelHandlerContext ctx,HttpX509Request req,String txt,boolean forceClose) {
              sendTxt(ctx,req,txt,HttpResponseStatus.OK,forceClose);
       }
       public void sendTxt(ChannelHandlerContext ctx,HttpX509Request req,String txt,HttpResponseStatus ret,boolean forceClose) {
       ...
       public void sendJSON(ChannelHandlerContext ctx,HttpX509Request req,String txt) {
       ...
       public void getPost(ChannelHandlerContext ctx,HttpX509Request req) {
              req.clearDecoder();

              lStore  store =lRoot.getStore("av");
              lFolder folder=store.getFolder("outbox");

              folder.open(lFolder.WRITE);

              //lFSMessage msg_xml=((lFSFolder)folder).createMessageXML();

              lMessage  m=req.getUploadMsg();
              //lMessage   msg=msg_xml.get();
              //msg.set(m);
              //msg_xml.save();
              folder.save(m);
       ...
       public void getMsg(ChannelHandlerContext ctx,HttpX509Request req) {
              ...
              if(store==null){
                 sendTxt(ctx,req,_getError("cmd:"+req.getCmd()+" user:"+req.getStore()+" unknow"),true);return;
              }
              lFolder folder=store.getFolder(req.getFolder());
              if(folder==null){
                 sendTxt(ctx,req,_getError("cmd:"+req.getCmd()+" user:"+req.getStore()+" folder:"+req.getFolder()+" unknow"),true);return;
              }
              folder.open(lFolder.READ_ONLY);
      
              lMessage msg=folder.getMsg().get(0);/**/
              if(msg==null){
                 sendTxt(ctx,req,_getError("cmd:"+req.getCmd()+" user:"+req.getStore()+" folder:"+req.getFolder()+" msg:"+req.getMsg()+" unknown"),true);return;
              }
              //logger.trace(msg);
              FullHttpResponse response;
              ...
              if(msg.getBodyBin()!=null){
                 ...
                 sendTxt(ctx,req,_getError("cmd:"+req.getCmd()+" user:"+req.getStore()+" folder:"+req.getFolder()+" empty"),true);return;
              }
       ...
       public void getMenu(ChannelHandlerContext ctx,HttpX509Request req) {
              String r=_getMenu(req);
              if(r==null)return;
              sendTxt(ctx,req,r,false);
       }
       private String _getMenu(HttpX509Request req) {
              if(req.getCmd().equals("menu")==false){ 
                 return _getError("cmd:"+req.getCmd()+" no op");
              }
              ...
              buf.append("<html>");
              buf.append("<head>");
              buf.append("<title>littlehttp:MENU</title>\r\n");
              buf.append("</head>\r\n");
              buf.append("<body bgcolor=white><style>td{font-size: 12pt;}</style>");
              // POST with enctype="multipart/form-data"
              buf.append("<CENTER>Send FILE (POST MULTIPART FORM)<HR WIDTH=\"100%\" NOSHADE color=\"blue\"></CENTER>");
              buf.append("<FORM ACTION=\"/post\" ENCTYPE=\"multipart/form-data\" METHOD=\"POST\">");
              buf.append("<input type=hidden name=getform value=\"POST\">");
              buf.append("<table border=\"0\" WIDTH=\"100%\" >");
              buf.append("<tr><td>TO:</td><td><input type=text name=\"to\" size=180></td></tr>");
              buf.append("<tr><td>TO:</td><td><input type=text name=\"to\" size=180></td></tr>");
              buf.append("<tr><td>TO:</td><td><input type=text name=\"to\" size=180></td></tr>");
              buf.append("<tr><td>FROM:</td><td><input type=text name=\"from\" size=180>");
              buf.append("<tr><td>SUBJECT:</td><td><input type=text name=\"subject\" size=180>");
              buf.append("<tr><td>File:</td><td><input type=file name=\"myfile\"></td></tr>");
              buf.append("<tr><td><INPUT TYPE=\"submit\" NAME=\"Send\" VALUE=\"Send\"></INPUT></td>");
              buf.append("<td><INPUT TYPE=\"reset\" NAME=\"Clear\" VALUE=\"Clear\" ></INPUT></td></tr>");
       ...
       public void getInfo(ChannelHandlerContext ctx,HttpX509Request req) {
              String r=_getInfo(req);
              if(r==null)return;
              sendTxt(ctx,req,r,false);
       }
       private String _getInfo(HttpX509Request req) {
              StringBuilder buf=new StringBuilder(10240);
              buf.append("<html>");
              buf.append("<head>");
              buf.append("<title>littlehttp:ROOT</title>\r\n");
              buf.append("</head>\r\n");
              buf.append("<body bgcolor=white><style>td{font-size: 12pt;}</style>");
              buf.append("<pre>WELCOME TO THE WILD WILD WEB SERVER\r\n");
              buf.append("===================================\r\n");
